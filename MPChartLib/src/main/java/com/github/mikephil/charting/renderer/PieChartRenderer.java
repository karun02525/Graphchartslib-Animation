        ...
        for (int j = 0; j < entryCount; j++) {
            ...
            if(j==0)
                mRenderPaint.setShader(
                        new RadialGradient(center.x, center.y, radius, dataSet.getColor(1), dataSet.getColor(0),
                                Shader.TileMode.CLAMP));
            else
                mRenderPaint.setShader(
                        new RadialGradient(center.x, center.y, radius, dataSet.getColor(2), dataSet.getColor(3),
                                Shader.TileMode.CLAMP));
    ...
    public void drawHighlighted(Canvas c, Highlight[] indices) {
        ...
        for (int i = 0; i < indices.length; i++) {
         ...
         //   mRenderPaint.setColor(highlightColor);
            ...
            if(highlightColor==-399959)
                mRenderPaint.setShader(
                        new RadialGradient(center.x, center.y, radius, mChart.getData().getDataSet().getColor(2), mChart.getData().getDataSet().getColor(3),
                                Shader.TileMode.CLAMP));

            else
                mRenderPaint.setShader(
                        new RadialGradient(center.x, center.y, radius, mChart.getData().getDataSet().getColor(1), mChart.getData().getDataSet().getColor(0),
                                Shader.TileMode.CLAMP));
