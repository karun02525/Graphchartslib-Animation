...
public class CandleStickChartRenderer extends LineScatterCandleRadarRenderer {
    ...
    protected void drawDataSet(Canvas c, ICandleDataSet dataSet) {
        ...
        int[]mixcolors = new int[]{
                dataSet.getColor(dataSet.getColors().size()-4),
                dataSet.getColor(dataSet.getColors().size()-2),
                dataSet.getColor(dataSet.getColors().size()-1),
                dataSet.getColor(dataSet.getColors().size()-3)
        };

        int[]normalColor =new int[]{
                dataSet.getColor(dataSet.getColors().size()-2),
                dataSet.getColor(dataSet.getColors().size()-4),
                dataSet.getColor(dataSet.getColors().size()-4)
            ...
            if (showCandleBar) {
                ...
                if (open > close) { // decreasing
                   ...
                   /* c.drawRect(
                            ...
                            mRenderPaint);*/
                    if(dataSet.getColor(j) == -3044758)
                        mRenderPaint.setShader(new LinearGradient(mBodyBuffers[0], mBodyBuffers[3],  mBodyBuffers[2], mBodyBuffers[1], normalColor, null, Shader.TileMode.MIRROR));
                    else if(dataSet.getColor(j) == -14062629)
                        mRenderPaint.setShader(new LinearGradient(mBodyBuffers[0], mBodyBuffers[3],  mBodyBuffers[2], mBodyBuffers[1], fastColor, null, Shader.TileMode.MIRROR));
                    else
                        mRenderPaint.setShader(new LinearGradient(mBodyBuffers[0], mBodyBuffers[3],  mBodyBuffers[2], mBodyBuffers[1], mixcolors, null, Shader.TileMode.MIRROR));
